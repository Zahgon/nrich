...
public class AesCsrfTokenManagerService implements CsrfTokenManagerService {
    ...
    private byte[] encryptedCurrentTimeBytes(Key key, byte[] currentTimeBytes) {
        ...
        return ByteBuffer.allocate(initializationVector.length + encryptedCurrentTimeInBytes.length).put(initializationVector).put(encryptedCurrentTimeInBytes).array();
    }
