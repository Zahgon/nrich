...
public class DefaultRegistryDataService implements RegistryDataService {
    ...
    private Map<String, JpaQueryBuilder<?>> initializeQueryBuilderMap(RegistryDataConfigurationHolder registryDataConfigurationHolder) {
        return registryDataConfigurationHolder.registryDataConfigurationList().stream().collect(Collectors.toMap(registryDataConfiguration -> registryDataConfiguration.registryType().getName(), registryDataConfiguration -> new JpaQueryBuilder<>(entityManager, registryDataConfiguration.registryType())));
    }
    ...
    private Map<String, Object> resolveSearchRequestMap(ManagedTypeWrapper managedTypeWrapper, ListRegistryRequest request, SearchPropertyConfiguration searchPropertyConfiguration) {
        ...
        if (request.searchParameter() != null) {
            searchRequestMap = stringToEntityPropertyMapConverter.convert(request.searchParameter().query(), request.searchParameter().propertyNameList(), managedTypeWrapper.getIdentifiableType(), searchPropertyConfiguration);
        }
    ...
    private void clearAssociationValues(ManagedTypeWrapper managedTypeWrapper, Object instance, Object entityData) {
        Set<String> associationList = managedTypeWrapper.getSingularAssociationList().stream().map(association -> PathResolvingUtil.convertToPathList(association.path())[0]).collect(Collectors.toSet());
